...
if __name__ == "__main__":
    ...
    hiring_dataset = load_dataset("buseskorkmaz/hiring_w_q_context_256_filtered")["train"]
    items = [row for row in hiring_dataset]
    prompt2idx = {remove_links(items[idx]['prompt']): idx for idx in range(len(items))}
    ...
    original_evaluation_dataset = evaluation_dataset.map(lambda x: evaluator.calc_q_value(x, x["text"]))
    
    generated_evaluation_dataset.save_to_disk(f"buseskorkmaz/{args.save_path}_paper_generated")
    original_evaluation_dataset.save_to_disk(f"buseskorkmaz/{args.save_path}__paper_original")
