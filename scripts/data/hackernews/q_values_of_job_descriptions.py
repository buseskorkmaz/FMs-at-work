...
def extract_info(row):
    ...
    technologies = re.search(r'(Technologies|Technologies and Techniques)\s*:(.*?)(?=\n[^-]|$)', text, re.DOTALL)
    technologies = technologies.group(2).strip() if technologies else 'Unknown'
    ...
    location = location.replace("/", " ")
    if any(term.lower() in location.lower() for term in us_states) or any(
    ...
    return {'Location': location, 'Technologies': technologies, 'Remote': remote, 'Relocate': relocate, 'Main_location': main_location}
...
def get_element_percentages(lst):
    ...
    for element, count in element_counts.items():
        ...
        element_percentages[element] = percentage
...
def calc_q_value(job_desc):
    ...
    filtered_user_profiles = user_profile_dataset.filter(lambda x: filter_candidates(x,job_desc["location"]))
