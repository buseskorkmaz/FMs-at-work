...
class WorkableEnvironment(Language_Environment):
    ...
    def step(self, action: str) -> Tuple[WorkableObservation, float, bool]:
        if self.stepped:
        ...
        return WorkableObservation(self.parent, action, reward), reward, True

    def reset(self) -> WorkableObservation:
        self.stepped = False
        self.parent = None
        if self.include_parent:
            self.parent = random.choice(self.data)[0][0]
        return WorkableObservation(self.parent, None, None)
