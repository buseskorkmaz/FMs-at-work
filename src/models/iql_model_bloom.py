...
class PerTokenIQL(BaseTransformer):
    def __init__(self, 
        ...
        assert isinstance(model, PreTrainedModel) or isinstance(model, BloomForCausalLM)
    ...
    def forward(self, 
        ...
        elif isinstance(self.model, BloomForCausalLM):
...
class IQL_Policy(Policy):
    ...
    #     max_len = min(max_len, self.iql_model.model.config.max_length)
    ...
    def beam_raw(self, 
        ...
        if max_length is None:
            max_length = self.iql_model.model.config.max_length
        max_length = min(max_length, self.iql_model.model.config.max_length)
        device = self.iql_model.device
