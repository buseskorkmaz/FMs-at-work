...
class BC_LM(BaseTransformer):
    def __init__(self, 
        ...
        assert isinstance(model, BloomForCausalLM) or isinstance(model, PreTrainedModel)
...
class BC_Policy(Policy):
    ...
    def sample_raw(self, 
        ...
        if max_length is None:
            max_length = self.bc_lm.model.config.max_length
        max_length = min(max_length, self.bc_lm.model.config.max_length)
