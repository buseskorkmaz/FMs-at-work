import json
from models.bc_lm_llama import BC_LM, BC_Evaluator, BC_Policy
import torch
...
from models.iql_model_llama import IQL_Evaluator, IQL_Policy, PerTokenIQL, TopAdvantageNGrams
...
def load_chai_model(config, device, verbose=True):
    ...
    if config['use_cache']:
        ...
        cache = None
    model = ChaiModel(dataset, device, llama, config['alpha'], config['gamma'], cache)
    return load_model(config['load'], model, device, verbose=verbose)
...
def load_dt_model(config, device, verbose=True):
    llama = load_item(config['llama'], verbose=verbose)
    dataset = load_item(config['dataset'], device, verbose=verbose)
    model = DT(llama, dataset, device, config['transition_weight'])
    return load_model(config['load'], model, device, verbose=verbose)
