...
from models.iql_model_flant5 import IQL_Evaluator, IQL_Policy, PerTokenIQL, TopAdvantageNGrams
...
from transformers import PreTrainedModel, T5ForConditionalGeneration, T5Config
...
@register('flant5')
def load_flant5(config, verbose=True):
    obj = T5ForConditionalGeneration if config['lm_head'] else PreTrainedModel
    if config['from_pretrained']:
        model = obj.from_pretrained(config['flant5_type'])
        print(f"--> Model FlanT5")
        total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print(f"\n--> FlanT5 has {total_params / 1e6} Million params\n")
        return model
    config = T5Config.from_pretrained(config['flant5_type'])
    return obj(config)

@register('bc_lm')
def load_bc_lm(config, device, verbose=True):
    flant5 = load_item(config['flant5'], verbose=verbose)
    dataset = load_item(config['dataset'], device, verbose=verbose)
    model = BC_LM(flant5, dataset, device, config['transition_weight'])
...
@register('per_token_iql_flant5')
def load_per_token_iql_flant5(config, device, verbose=True):
    flant5 = load_item(config['flant5'], verbose=verbose)
    dataset = load_item(config['dataset'], device, verbose=verbose)
    model = PerTokenIQL(flant5, dataset, device, config['alpha'], config['gamma'], 
...
def load_per_token_cql(config, device, verbose=True):
    flant5 = load_item(config['flant5'], verbose=verbose)
    dataset = load_item(config['dataset'], device, verbose=verbose)
    model = CQLModel(flant5, dataset, device, config['alpha'], config['gamma'], 
                     config['beta'], config['transition_weight'], config['clip_weight'], 
                     config['value_max'], config['value_min'], config['detach_v'], 
                     config['detach_pi'], config['detach_q'], config['double_q'], 
...
def load_per_token_bcq(config, device, verbose=True):
    flant5 = load_item(config['flant5'], verbose=verbose)
    dataset = load_item(config['dataset'], device, verbose=verbose)
    model = BCQModel(flant5, dataset, device, config['alpha'], config['gamma'], 
                     config['beta'], config['transition_weight'], config['clip_weight'], 
                     config['value_max'], config['value_min'], config['detach_v'], 
                     config['detach_pi'], config['detach_q'], config['double_q'], 
                     config['seperate_policy'], config['seperate_target'], config['exp_weights'], 
...
def load_per_token_psi(config, device, verbose=True):
    flant5 = load_item(config['flant5'], verbose=verbose)
    dataset = load_item(config['dataset'], device, verbose=verbose)
    model = PsiModel(flant5, dataset, device, config['alpha'], config['gamma'], 
                     config['beta'], config['transition_weight'], config['clip_weight'], 
                     config['value_max'], config['value_min'], config['detach_v'], 
                     config['detach_pi'], config['detach_q'], config['double_q'], 
                     config['seperate_policy'], config['seperate_target'], config['exp_weights'], 
...
def load_per_token_g(config, device, verbose=True):
    flant5 = load_item(config['flant5'], verbose=verbose)
    dataset = load_item(config['dataset'], device, verbose=verbose)
    model = GModel(flant5, dataset, device, config['alpha'], config['gamma'], 
                   config['beta'], config['transition_weight'], config['clip_weight'], 
                   config['value_max'], config['value_min'], config['detach_v'], 
                   config['detach_pi'], config['detach_q'], config['double_q'], 
                   config['seperate_policy'], config['seperate_target'], config['exp_weights'], 
...
def load_per_utterance_iql(config, device, verbose=True):
    flant5 = load_item(config['flant5'], verbose=verbose)
    dataset = load_item(config['dataset'], device, verbose=verbose)
    model = PerUtteranceIQL(flant5, dataset, device, config['alpha'], config['gamma'], 
                            config['beta'], config['transition_weight'], config['clip_weight'], 
                            config['value_max'], config['value_min'], config['detach_v'], 
                            config['detach_pi'], config['detach_q'], config['double_q'], 
                            config['tau'], config['seperate_policy'], config['seperate_target'], 
...
def load_chai_model(config, device, verbose=True):
    ...
    if config['use_cache']:
        ...
        cache = None
    model = ChaiModel(dataset, device, flant5, config['alpha'], config['gamma'], cache)
    return load_model(config['load'], model, device, verbose=verbose)
...
def load_dt_model(config, device, verbose=True):
    flant5 = load_item(config['flant5'], verbose=verbose)
    dataset = load_item(config['dataset'], device, verbose=verbose)
    model = DT(flant5, dataset, device, config['transition_weight'])
    return load_model(config['load'], model, device, verbose=verbose)
